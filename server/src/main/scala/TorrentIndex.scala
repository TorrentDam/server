...
object TorrentIndex {
  ...
  def apply()(using logger: Logger[IO]): Resource[IO, TorrentIndex] = {
    Resource.eval {
      for
          ...
          index.entries
            ...
            .map {
              case (searchField, entry) =>
                val score = words.map(word => if searchField.contains(word) then word.length else 0).sum
                (entry, score)
            }
            .filter(_._2 > 0)
            .toList
  ...
  object Entry {
    given jsonRW: upickle.default.ReadWriter[Entry] = upickle.default.macroRW
  }
}
